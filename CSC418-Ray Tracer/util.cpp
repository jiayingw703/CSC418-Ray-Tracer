...
Colour& Colour::operator*=(const double factor) {
	m_data[0] *= factor;
	m_data[1] *= factor;
	m_data[2] *= factor;
	return *this;
...
Colour Material::getTextureColor(const Ray3D& ray) {
	if (texture) return texture->getColor(ray.intersection.uvCoord);
	else return Colour(1.0, 1.0, 1.0);
}


Texture::Texture(std::string filename) : width(0), height(0), rarray(NULL), garray(NULL), barray(NULL) {
	if (!bmp_read((char *)filename.c_str(), &width, &height, &rarray, &garray, &barray))
	{
		// throw "Bad texture map";
	}
}
...
Colour Texture::getColor(const Point3D& uvCoord) {
	unsigned int x = ((unsigned int)(uvCoord[0] * width)) % width;
	unsigned int y = ((unsigned int)(uvCoord[1] * height)) % height;

	return Colour(rarray[y * width + x] / MAXRGB, garray[y * width + x] / MAXRGB, barray[y * width + x] / MAXRGB);
}